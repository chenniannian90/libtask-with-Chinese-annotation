...
altcanexec(Alt *a)
{
	...
	// 如果channel 缓存区为0, 对channel的读,检查写队列是否有数据;对channel的写,检查读队列是否有数据
	if(c->bufsize == 0){
...
altcopy(Alt *s, Alt *r)
{
	...
	// 如果发送者的op为接收, 说明发送和接收者反了, 做一下交互 保证s为发送者
	if(s->op == CHANRCV){
		t = s;
		s = r;
		r = t;
	 ...
	 */
	// 如果channel不支持缓存或者为空，直接发发送数据转移到接收数据
	if(s && r && c->nbuf == 0){
